...
describe("tool-result-serializer", () => {
  ...
  describe("emby envelope handling", () => {
    it("summarizes envelope-style results with lowercase keys", () => {
      ...
      expect(parsed.compact).toBeUndefined(); // compact flag not passed through
    });

    it("handles emby_library tool", () => {
      const input = JSON.stringify({
        count: 1,
        returned: 1,
        hasMore: false,
  ...
  describe("edge cases", () => {
    it("handles empty items array", () => {
      const input = JSON.stringify({ count: 0, returned: 0, hasMore: false, items: [] });
      const result = summarizeToolResult("emby_search", input);
      const parsed = JSON.parse(result);
