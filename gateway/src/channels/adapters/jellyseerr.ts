// Jellyseerr integration adapter (non-messaging)
...
export function createJellyseerrAdapter(channelId: string): ChannelAdapter {
  ...
  const adapter: ChannelAdapter = {
    ...
    async connect(config) {
      ...
      try {
        ...
        await fetchJson(`${base}/api/v1/status`, apiKey);
        setStatus("connected");
      } catch (err) {
    ...
    async send() {
      throw new Error("Jellyseerr is a browsing integration and does not support send()");
    },
